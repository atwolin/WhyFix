...
PATHS = FilePath()
RETRIEVAL_DOC = load_method_config("retrieval")
...
def create_or_get_retrivers(top_k: int = 10):
    # Vector-based retrieval setup
    vectorstore_name = "simplified_collocation_collection"
    vectorstore_path = os.path.join(PATHS.folderPath_collocation_faiss, vectorstore_name)
    collocation_store = get_vectorstore(vectorstore_name, vectorstore_path)
...
def get_ensemble_retriever(fusion: bool):
    faiss_retriever, bm25_retriever = create_or_get_retrivers()
...
def get_full_collocation_info(simplified_collocations):
    ...
    full_data = []
    with open(PATHS.filePath_collocation_full_txt, 'r', encoding='utf-8') as f:
        for line in f:
            if line.strip():
...
def get_other_category_collocation(collocations, pivot_word_lemma, pivot_word_pos):
    df_full_collocation = pd.read_csv(PATHS.filePath_collocation_full_csv)
...
def self_rag(retriever, formattd_sentences: str, learner_sentences: str):
    ...
    if retrieval_decision == "yes":
        ...
        print(f"{'-' * 50}\nStep 3: Evaluate relevance of retrieved documents")
        relevant_contexts = []
        for i, context_item in tqdm(enumerate(full_retrieved_collocations), desc="Evaluating relevance"):  # Iterate over each retrieved doc
...
def find_collocations_fce_and_longman():
    if len(sys.argv) > 2:
    ...
    retriever = get_ensemble_retriever(IS_FUSION)

    df_fce = pd.read_csv(PATHS.filePath_fce_sample)
    df_longman = pd.read_csv(PATHS.filePath_longman_sample)
...
def find_collocations_runtime(model_type='gpt', is_fusion=True):
    input_path = PATHS.filePath_runtime_dictInfo
    output_path = PATHS.filePath_runtime_withCollocation
    data_name = "Runtime"
    ...
    process_one_df(df, data_name, retriever, output_path)

    df_with_collocations = pd.read_json(output_path, orient='records')
    data_with_collocations = df_with_collocations.to_dict(orient='records')[0]

    return data_with_collocations


if __name__ == "__main__":
    find_collocations_fce_and_longman()
