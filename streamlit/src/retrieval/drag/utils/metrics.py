...
class RAGMetrics:
    ...
    def update(
        ...
        bleu = sentence_bleu([ground_truth.strip().lower().split()], prediction.strip().lower().split())
        self.metrics['bleu'].append(bleu)

        # Calculate ROUGE-L score
        rouge_scorer_instance = rouge_scorer.RougeScorer(['rougeL'], use_stemmer=True)
        rouge_l = rouge_scorer_instance.score(ground_truth.strip().lower(), prediction.strip().lower())['rougeL'].fmeasure
        self.metrics['rouge_l'].append(rouge_l)

        # Calculate METEOR score
        meteor = meteor_score(
            [ground_truth.strip().lower().split()],
            prediction.strip().lower().split()
        )
        self.metrics['meteor'].append(meteor)

        # Calculate BERTScore
        P, R, F1 = bert_score([prediction], [ground_truth], lang="en", verbose=False)
        self.metrics['bert_score'].append(F1.mean().item())
