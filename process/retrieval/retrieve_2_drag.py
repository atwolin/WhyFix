...
SM = SupportMaterial(TOPIC, EMBEDDING_TYPE)


def load_knowledge_documents(path_vectorstore, vectorstore, chunk_settings) -> List[Document]:
    """Load L2 knowledge documents"""
    files = os.listdir(SM.folderPath_all)
    for file in tqdm(files, desc="Loading documents"):
        path_file = os.path.join(SM.folderPath_all, file)
        vectorstore = encode_pdf(path_file, path_vectorstore, vectorstore, chunk_settings)
    # return vectorstore.docstore  # Limit for example
...
def main():
    ...
    program_setting_doc = load_yaml("/home/nlplab/atwolin/thesis/code/process/utils/retrieve_query.yaml")

    model_settings = program_setting_doc["model"][MODEL_TYPE]['settings']
    chunk_settings = program_setting_doc["chunking"][str(CHUNKING_TYPE)]
    embedding_settings = program_setting_doc["embedding_model"][str(EMBEDDING_TYPE)]
    chunk_settings["length_function"] = len
    query = program_setting_doc["query_retrieval"]["rewrite"][TOPIC]["fourOneNano"]
    ...
    path_vectorstore = os.path.join(SM.folderPath_l2_faiss, f"knowledge_{TOPIC}_{CHUNKING_TYPE}")
    ...
    for dataset_name in datasets:
        ...
        context_lengths = [1000000]  # Reduced for testing
    ...
    save_payload_to_json(drag_payload, SM.filePath_result_drag)
    ...
    save_payload_to_json(iter_payload, SM.filePath_result_iter_drag)
